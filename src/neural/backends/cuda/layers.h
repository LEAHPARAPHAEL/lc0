...
#ifdef USE_CUDNN
template <typename DataType>
class DepthwiseLegacy : public BaseLayer<DataType> {
  using BaseLayer<DataType>::C;
  using BaseLayer<DataType>::H;
  using BaseLayer<DataType>::W;
  using BaseLayer<DataType>::GetC;
 ...
 public:
  DepthwiseLegacy(BaseLayer<DataType>* ip, int C, int H, int W, int size,
            ActivationFunction activation = ACTIVATION_NONE, bool bias = false);

  DepthwiseLegacy(bool nhwc, int C, int H, int W, int size,
            ActivationFunction activation = ACTIVATION_NONE, bool bias = false);
