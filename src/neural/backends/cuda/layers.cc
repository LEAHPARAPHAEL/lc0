...
DepthwiseConvLayer<DataType>::DepthwiseConvLayer(
    ...
    ActivationFunction act, bool use_gemm_ex,  int min_batch_size, int max_batch_size, 
    cudnnHandle_t cudnn)
    : BaseLayer<DataType>(channels, height, width, prev, false, use_gemm_ex),
      channels_(channels),
      act_(act) {
  ...
  //int min_padded_batch = GetPaddedBatchSize(min_batch_size);
  //int max_padded_batch = GetPaddedBatchSize(max_batch_size);

  //for (int b = min_padded_batch; b <= max_padded_batch; b *= 2) {
  //    EnsureGraph(b, cudnn);
  //}
}
...
void DepthwiseConvLayer<DataType>::LoadWeights(float* weights,
  ...
  if (channels_ > 0 && weights != nullptr) {
      std::cout << "\n=== DEBUG: DepthwiseConvLayer (cuDNN) | Channel 0 ===" << std::endl;
      std::cout << "Weights (5x5 = 25 elements):\n";
      for (int i = 0; i < 25; ++i) {
          std::cout << weights[i] << (i == 24 ? "" : ", ");
      }
      std::cout << "\nBias: ";
      if (biases) {
          std::cout << biases[0];
...
void DepthwiseConvLayer<DataType>::EnsureGraph(int padded_batch, cudnnHandle_t cudnn) {
  ...
  auto X = graph->tensor(cudnn_frontend::graph::Tensor_attributes()
      .set_name("X")
      .set_dim({padded_batch, channels_, 8, 8})
      .set_stride({channels_ * 64, 64, 8, 1}));
...
void DepthwiseConvLayer<DataType>::Eval(
  ...
  addBias_NCHW<DataType>(output, output, biases_, batch_size, channels_, 8, 8, act_, stream);
}
#endif
...
void Conv1Layer<float>::cublasSpecialMatrixMul(const float* A, const float* B,
  ...
  if (nhwc_) {
      ...
      if (use_gemm_ex_) {
          ReportCUBLASErrors(cublasGemmStridedBatchedEx(
              cublas, CUBLAS_OP_T, CUBLAS_OP_N, M, N, K, &floatOne, A, CUDA_R_32F, K, 0,
              B, CUDA_R_32F, K, K * N, &floatZero, Out, CUDA_R_32F, M, M * N,
              batchSize, CUDA_R_32F, CUBLAS_GEMM_DEFAULT));
      } else {
          ReportCUBLASErrors(cublasSgemmStridedBatched(
              cublas, CUBLAS_OP_T, CUBLAS_OP_N, M, N, K, &floatOne, A, K, 0,
              B, K, K * N, &floatZero, Out, M, M * N, batchSize));
      }
  } else {
      if (use_gemm_ex_) {
          ReportCUBLASErrors(cublasGemmStridedBatchedEx(
...
void Conv1Layer<DataType>::Eval(int N, DataType* output, const DataType* input,
  ...
  if (use_bias_) {
      if (nhwc_) {
          addVectors(output, output, biases_, N * C * H * W, N * C * H * W, C, act_, stream);
      } else {
          addBias_NCHW(output, output, biases_, N, C, H, W, act_, stream);
      }
  } else if (act_ != ACTIVATION_NONE) {
