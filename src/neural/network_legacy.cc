...
BaseWeights::BaseWeights(const pblczero::Weights& weights)
  ...
  for (const auto& block : weights.tower()) {
...
BaseWeights::TowerBlock::TowerBlock(
    ...
    : dense_w(LayerAdapter(pb_block.dense_w()).as_vector()),
      dense_b(LayerAdapter(pb_block.dense_b()).as_vector()),
      mult_gate(LayerAdapter(pb_block.mult_gate()).as_vector()),
      add_gate(LayerAdapter(pb_block.add_gate()).as_vector()),
      ln_gammas(LayerAdapter(pb_block.ln_gammas()).as_vector()),
      ln_betas(LayerAdapter(pb_block.ln_betas()).as_vector()),
